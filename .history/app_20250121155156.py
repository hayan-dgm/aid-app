...
def token_required(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        token_creation_time = jwt_data['iat']
        
        conn = get_db_connection()
        session = conn.execute('SELECT token_creation_time FROM active_sessions WHERE user_id = ?', (user_id,)).fetchone()
        conn.close()
        
        if session and session['token_creation_time'] > token_creation_time:
...
#             conn = get_db_connection()
#             existing_session = conn.execute('SELECT * FROM active_sessions WHERE user_id = ?', (user['id'],)).fetchone()
...
#             conn.execute('INSERT INTO active_sessions (user_id, access_token) VALUES (?, ?)', (user['id'], access_token))
#             conn.commit()
#             conn.close()
...
def login():
    try:
        ...
        if user:
            access_token = create_access_token(identity={'id': user['id'], 'username': user['username'], 'isAdmin': user['isAdmin']})
            token_creation_time = datetime.datetime.utcnow()
            ...
            conn.execute('INSERT INTO active_sessions (user_id, access_token, token_creation_time) VALUES (?, ?, ?)', (user['id'], access_token, token_creation_time))
            conn.commit()
            conn.close()
