...
#         if session and session['login_time'] > login_time:
...
def token_required(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if session:
            db_login_time = datetime.strptime(session['login_time'], "%Y-%m-%d %H:%M:%S")
            token_login_time = datetime.fromtimestamp(login_time)
            if db_login_time > token_login_time:
                return jsonify({'message': 'Token has been revoked'}), 401
...
def login():
    try:
        ...
        if user:
            access_token = create_access_token(identity={'id': user['id'], 'username': user['username'], 'isAdmin': user['isAdmin']})
            login_time = datetime.datetime.now(datetime.utcnow)
            ...
            conn.execute('INSERT INTO active_sessions (user_id, access_token, login_time) VALUES (?, ?, ?)', (user['id'], access_token, login_time))
            conn.commit()
            conn.close()
