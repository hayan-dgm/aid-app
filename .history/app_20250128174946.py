...
def login():
    try:
        ...
        if user:
            ...
            access_token = create_access_token(identity={'id': user_dict['id'], 'username': user['username'], 'isAdmin': user['isAdmin']})
            login_time = datetime.now(timezone.utc) # Use datetime.now(timezone.utc)
            conn = get_db_connection()
            existing_session = conn.execute('SELECT * FROM active_sessions WHERE user_id = ?', (user_dict['id'],)).fetchone()
            
            if existing_session:
                conn.execute('DELETE FROM active_sessions WHERE user_id = ?', (user_dict['id'],))
            
            conn.execute('INSERT INTO active_sessions (user_id, access_token, login_time) VALUES (?, ?, ?)', (user_dict['id'], access_token, login_time))
            conn.commit()
            conn.close()
